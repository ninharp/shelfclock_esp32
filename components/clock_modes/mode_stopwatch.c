...
static bool s_stopwatch_ended = false;
...
void mode_stopwatch_start(int32_t duration_ms) {
    ...
    s_stopwatch_ended  = false;
}

bool mode_stopwatch_check_ended(void) {
    if (s_stopwatch_ended) { s_stopwatch_ended = false; return true; }
    return false;
}
